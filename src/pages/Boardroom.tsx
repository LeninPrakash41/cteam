...
export function Boardroom() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-zinc-50 font-sans relative">
      ...
      {/* Quick Suggested Questions Bar (Always available even mid-conversation) */}
      {messages.length > 0 && (
        <div className="bg-white border-t border-zinc-200/80 px-6 py-2.5 flex-shrink-0">
          <div className="max-w-4xl mx-auto flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5">
            <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1 flex-shrink-0 mr-1">
            ...
            {getSuggestedQuestions().map((sq, i) => (
              <button
                ...
                type="button"
                onClick={() => setSuggestedInput(sq.text)}
                className="flex items-center gap-1.5 px-3 py-1 bg-zinc-100 hover:bg-indigo-50 hover:text-indigo-700 hover:border-indigo-200 border border-zinc-200/80 rounded-full text-xs font-medium text-zinc-700 transition-all flex-shrink-0 whitespace-nowrap"
              >
                {sq.agent ? (
                  <img src={sq.agent.avatarUrl} alt={sq.agent.name} className="w-4 h-4 rounded-full object-cover" referrerPolicy="no-referrer" />
                ) : (
                  <span className="w-4 h-4 rounded-full bg-indigo-100 text-indigo-700 text-[9px] font-bold flex items-center justify-center">C</span>
                )}
                <span className="font-semibold text-indigo-600">{sq.role.split(' ')[0]}:</span>
                <span className="truncate max-w-[220px]">{sq.text.replace(/^@[^\s]+\s+/, '')}</span>
              </button>
            ))}
          </div>
        </div>
